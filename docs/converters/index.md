...
A CZI scene can itself be a mosaic (the CZI `M` dimension), i.e. a set of overlapping sub-tiles. `Mosaic Mode` controls how these are handled:

| Mode | Description |
|---|---|
| `tiles` (default) | Each mosaic sub-tile becomes its own positioned field of view (`P1_m0`, `P1_m1`, ...) and `ome-zarr-converters-tools` stitches them together. |
| `assembled` | Each scene is converted as a single field of view; `czifile` assembles the mosaic when reading the scene. |

Use `tiles` if you want the converter to perform stitching/alignment across mosaic sub-tiles; use `assembled` if you prefer the reader to return the pre-assembled mosaic as one image.

### Acquisition Options (Advanced)

The `Advanced` field accepts an `AcquisitionOptions` object covering per-acquisition overrides and filtering. Most users only need `Condition Table Path` here.
